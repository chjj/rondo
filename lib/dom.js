...
DOM.implement = function(key, func) {
  if (typeof key === 'object') {
    return each(key, function(val, key) {
      DOM.implement(key, val);
    });
  }
...
DOM.env = (function() {
  ...
  (function EXECUTE_TEST(browser, test) {
    if (test()) {
      env[browser] = true;
      return noop;
    }
...
(function IMPLEMENT_TRAVERSAL(name, func) {
...
('descendant', function(el, test) {
  return DOM.descendants(el, test);
})

('ancestor', function(el, test) {
  return DOM.ancestors(el, test);
});

/*
('previous', function(el, test) {
  return DOM.above(el, test);
})

('next', function(el, test) {
  return DOM.below(el, test);
})

('child', function(el, test) {
  return DOM.children(el, test);
})

('parent', function(el, test) {
  return DOM.parents(el, test);
});
*/
...
DOM.implement({
  ...
  hasSibling: function(el, sub) {
    return !!~DOM.siblings(el).indexOf(sub);
  }
});
...
var flstyle = function(el, name) {
  return parseFloat(DOM.getStyle(el, name)) || 0;
};

(function IMPLEMENT_DIMENSIONS(type, side, other) {
  ...
  var total = function(el, prop) {
    return flstyle(el, prop + '-' + side)
         + flstyle(el, prop + '-' + other);
  };
  ...
  DOM.implement(type, function(el, box) {
    box = box || 'content';
...
(function IMPLEMENT_SIDES(type, side, unit) {
  ...
  DOM.implement({
    ...
    setStyle: function(el, prop, val) {
      if (typeof prop === 'object') {
        return each(prop, function(prop, key) {
          DOM.setStyle(el, key, prop);
        });
      }
...
(function IMPLEMENT_SET_GET(name) {
...
DOM.map = function(obj, func) {
  DOM.each(obj, function(val, key) {
    obj[key] = func(val, key);
  });
};

DOM.filter = function(obj, func) {
...
DOM.pluck = function(obj, prop) {
  var res = [];
  DOM.each(obj, function(el) {
    res.push(el[prop]);
  });
  return res;
};

DOM.prototype.each = function(func, context) {
  DOM.each(this.dom, func, context || this);
  return this;
};
...
DOM.prototype.filter = function(func) {
  return DOM(DOM.filter(this.dom, func));
};

DOM.prototype.indexOf = function(el, start) {
  el = normalize(el);
  return DOM.indexOf(this.dom, el, start);
};

DOM.prototype.has = function(el) {
...
DOM.prototype.length = function(length) {
  if (arguments.length) {
    return this.dom.length = length;
  }
...
// need to be separate
DOM.load = function(el, url) {
  require('io').get(url, function(err, data) {
    if (!err) DOM.setContent(el, data);
  });
};
...
DOM.render = function(el, id, locals) {
  require('app').render(el, id, locals);
};

// shouldnt need this
DOM.prototype.render = function(id, locals) {
  require('app').render(this.dom[0], id, locals);
};

DOM.prototype.copy = function() {
  return DOM(this.dom.slice());
};
