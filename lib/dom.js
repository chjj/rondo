...
each({
  ...
  DOM.implement(name, function(el, index, type) {
    index = index || 1;
    ...
    if (typeof index === 'string') {
      type = index;
      while (el.nodeName.toLowerCase() !== type) {
        if (!(el = func(el))) break;
      }
    } else {
      while (index-- && el.nodeName.toLowerCase() !== type) {
        if (!(el = func(el))) break;
      }
    }
...
DOM.implement({
  ...
  addClass: function(el, name) {
    var cl = DOM.getProperty(el, 'className');
    DOM.setProperty(el, 'className', (cl ? cl + ' ' : '') + name);
  },
...
DOM.prototype.animate = (function() {
  ...
  return function(opt, func) {
    ...
    // bind
    this.once('transitionend webkitTransitionEnd OTransitionEnd', func);
  };
})();
...
if (function() {
  ...
  var HTML5 = ('article aside audio canvas details figcaption'
