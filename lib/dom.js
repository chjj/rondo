...
DOM.implement({
  children: function(el, test) {
    ...
    if (test) out = DOM.filter(out, test);
    return out;
  },
...
each({
  ...
  var total = function(el, prop) {
    return flstyle(el, prop + '-' + side[0])
           + flstyle(el, prop + '-' + side[1]);
  };
...
DOM.indexOf = function(obj, el, start) {
  var i = start || 0
    , l = obj.length;
...
DOM.prototype.length = function(length) {
  if (arguments.length) return this.length = length;
