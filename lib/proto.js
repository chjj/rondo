...
DOM._execute = function(key, type) {
  ...
  var func = DOM[key]
    , exec = DOM._execute[type];

  if (!exec) return;

  DOM.prototype[key] = function() {
    return exec.call(this, arguments, func);
  };
};
...
DOM._execute.set_all_dom = function(args_, func) {
  var args = slice.call(args_)
    , sub = args[1]
    , first = true;

  DOM._execute.set(args_, function() {
    args[1] = first 
      ? (first = false) || sub[1] 
      : sub.clone(true);
    func.apply(this, args);
